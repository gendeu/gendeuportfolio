...
if (carouselTrack && carouselItems.length) {
  ...
  // Initial centering: center first item or previously active
  window.addEventListener('load', () => {
    const savedIndex = Number(localStorage.getItem('carouselCenterIndex') || 0);
    const startItem = carouselItems[savedIndex] || carouselItems[0];
  ...
  if (prevBtn) prevBtn.addEventListener('click', () => {
    ...
    const idx = carouselItems.indexOf(active);
    const target = carouselItems[Math.max(0, idx - 1)];
    centerItem(target, true);
    localStorage.setItem('carouselCenterIndex', Math.max(0, idx - 1));
    setTimeout(updateActiveClasses, 180);
  });
  if (nextBtn) nextBtn.addEventListener('click', () => {
    const active = findCenteredItem(); if (!active) return;
    const idx = carouselItems.indexOf(active);
    const target = carouselItems[Math.min(carouselItems.length - 1, idx + 1)];
    centerItem(target, true);
    localStorage.setItem('carouselCenterIndex', Math.min(carouselItems.length - 1, idx + 1));
    setTimeout(updateActiveClasses, 180);
  });
  ...
  // Snap to nearest item when user stops scrolling (faster snap)
  ...
  carouselTrack.addEventListener('scroll', () => {
    ...
    }, 80); // 80ms after scroll stops -> snappier
  }, { passive: true });

  // On resize re-center the current active
  window.addEventListener('resize', () => {
    const active = findCenteredItem() || carouselItems[0];
